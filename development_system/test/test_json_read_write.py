...
def test_read_json_file(temp_json_file):
    success, file_content = JsonReadWrite.read_json_file(temp_json_file)
...
def test_read_json_file_nonexistent():
    ...
    success, file_content = JsonReadWrite.read_json_file(non_existent_file)
    assert success is False
    assert file_content is None

def test_update_json_file(temp_json_file):
    success = JsonReadWrite.update_json_file(temp_json_file, "key3", "value3")
    assert success is True

    _, file_content = JsonReadWrite.read_json_file(temp_json_file)
    assert file_content == {"key1": "value1", "key2": "value2", "key3": "value3"}

def test_update_json_file_error():
    error_file = "error_file.json"
    success = JsonReadWrite.update_json_file(error_file, "key3", "value3")
    assert success is False

def test_write_json_file(temp_json_file):
    new_content = {"key1": "updated_value1", "key4": "value4"}
    success = JsonReadWrite.write_json_file(temp_json_file, new_content)
    assert success is True
